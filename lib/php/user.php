...
class User{
	...
	// No se podra acceder a las variables de manera directa
	var $id;
	private $fbuser;
	private $twuser;
	private $name;
	...
	// Funcion get, asegura compatibilidad con Chevismo
	function get($what){
		return $this->g($what);	
	}
	...
	function sendPM($to,$msg,$thread=0,$ident=0,$com=0){
		// thread indica el hilo al que pertenece
		// from is always the current user
		//
		// $com : ID del comentario al que responde
		//
		// STATUS GUIDE
		//	0 -> Just sent, unread
		//	1 -> Read
	...
	function fb(){
		// Return fbuser if has Facebook linked
	...
	function tw(){
		// Return twuser if has Twitter linked
