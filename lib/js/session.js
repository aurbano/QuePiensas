...
/* ---------- AJAX  ------------- */
	...
	$('.paddedContent #loginForm').submit(function(event){
		...
		if($('#emailLog').val()=='' || $('#passLog').val()==''){
			$('#saveMsgErrorLog').text('Debes rellenar todos los campos').show();	
		}else{
			// Un poco de diseño jaja
			$('#saveMsgErrorLog').text('').hide();	
				  ...
				  function(data){
					  	if(data){
							if(data.done == 'false'){
								// No se pudo guardar:
								if(data.msg.length>0){
									$('#saveMsgErrorLog').html(stripslashes(data.msg)).show();
								}else{
									$('#saveMsgErrorLog').html('No ha sido posible guardar el mensaje, por favor int&eacute;ntalo m&aacute;s tarde.').show();
								}
							...
							$('#saveMsgErrorLog').html('Ha ocurrido un error, intentelo de nuevo mas tarde').show();
						}
				  }, "json");
		}
	});
